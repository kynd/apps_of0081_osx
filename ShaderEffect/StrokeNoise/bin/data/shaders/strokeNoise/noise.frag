...
float n(vec3 v) {
	return(simplexNoise3(v) * 0.5 + 0.5);
}
...
void main (void){
	...
	float n0 = time * 0.0021 + 12.3;
	float n1 = time * 0.00321 + 123.4;
	float n2 = time * 0.004321 + 1234.5;

	vec2 v0 = gl_TexCoord[0].st;
	vec2 v1 = gl_TexCoord[0].st;
	vec2 v2 = gl_TexCoord[0].st;
	v0 /= 1.5;
	v1 /= 82.0;
	v2 /= 183.0;
	
	float nr = (n(vec3(v0, n0)) + n(vec3(v1, n1)) + n(vec3(v2, n2))) / 3.0;
	float ng = (n(vec3(v0, n1)) + n(vec3(v1, n2)) + n(vec3(v2, n0))) / 3.0;
	float nb = (n(vec3(v0, n2)) + n(vec3(v1, n0)) + n(vec3(v2, n1))) / 3.0;
